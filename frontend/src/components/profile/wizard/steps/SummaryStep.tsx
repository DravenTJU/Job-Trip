      ...
      <div className="space-y-8">
            ...
            <div className="px-4 py-5 sm:p-6">
              <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  <dd className="mt-1 text-sm text-gray-900 dark:text-gray-100">{data.biography || '未填写'}</dd>
                </div>
              </dl>
            </div>
          </div>
              ...
              <div className="px-4 py-5 sm:p-6">
                <dl className="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                  <div className="sm:col-span-1">
                    ...
                    <dd className="mt-1 text-sm text-gray-900 dark:text-gray-100">{data.contactInfo.email || '未填写'}</dd>
                  </div>
                  
                  <div className="sm:col-span-1">
                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">电话</dt>
                    <dd className="mt-1 text-sm text-gray-900 dark:text-gray-100">{data.contactInfo.phone || '未填写'}</dd>
                  </div>
                  
                  <div className="sm:col-span-1">
                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">网站</dt>
                    <dd className="mt-1 text-sm text-gray-900 dark:text-gray-100">{data.contactInfo.website || '未填写'}</dd>
                  </div>
                  
                  <div className="sm:col-span-1">
                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">地址</dt>
                    <dd className="mt-1 text-sm text-gray-900 dark:text-gray-100">{data.contactInfo.address || '未填写'}</dd>
                  </div>
                </dl>
              </div>
            </div>
            ...
            <div className="px-4 py-5 sm:p-6">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                {data.educations && data.educations.length > 0 
                  ? `已添加 ${data.educations.length} 条教育经历` 
                  : '未添加教育经历'}
              </p>
            </div>
          </div>
            ...
            <div className="px-4 py-5 sm:p-6">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                {data.workExperiences && data.workExperiences.length > 0 
                  ? `已添加 ${data.workExperiences.length} 条工作经验` 
                  : '未添加工作经验'}
              </p>
            </div>
          </div>
            ...
            <div className="px-4 py-5 sm:p-6">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                {data.certifications && data.certifications.length > 0 
                  ? `已添加 ${data.certifications.length} 个证书` 
                  : '未添加证书'}
              </p>
            </div>
          </div>
            ...
            <div className="px-4 py-5 sm:p-6">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                {data.honorsAwards && data.honorsAwards.length > 0 
                  ? `已添加 ${data.honorsAwards.length} 项荣誉奖项` 
                  : '未添加荣誉奖项'}
              </p>
            </div>
          </div>
        ...
        <div className="bg-yellow-50/80 dark:bg-yellow-900/30 p-5 rounded-xl ring-2 ring-yellow-200 dark:ring-yellow-800/50 backdrop-blur-lg">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-200">请确认信息</h3>
              <div className="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                <p>
                  请确认以上信息准确无误，点击"完成创建"按钮创建您的个人档案。创建后您仍可随时修改和完善档案信息。
                </p>
              </div>
            </div>
          </div>
