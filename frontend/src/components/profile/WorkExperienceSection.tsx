...
const WorkExperienceSection: React.FC<WorkExperienceSectionProps> = ({ workExperiences }) => {
  ...
  const handleDelete = (workExperienceId: string) => {
    setExperienceToDelete(workExperienceId);
    setShowDeleteConfirm(true);
  };

  const confirmDelete = () => {
    if (experienceToDelete) {
      dispatch(deleteWorkExperience(experienceToDelete));
  ...
  return (
    <>
      ...
      <CustomConfirmDialog
        ...
        title={t('delete_work_experience', '删除工作经历')}
        message={t('confirm_delete_work', '确定要删除此工作经历吗？此操作无法撤销。')}
